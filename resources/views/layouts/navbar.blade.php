<nav class=" sticky w-full top-0 start-0 border-b border-gray-200">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 bg-default-bg">
        ...
        <div class="hidden w-full md:flex md:w-auto" id="navbar-multi-level">
            <ul class="flex flex-col md:items-center order-1 font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-primary-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-default-bg navbar-menu">
                @include('layouts.item-menu')
            </ul>

            <div class="gap-4 flex flex-row items-center justify-center order-2 md:ms-8 mt-4 md:mt-0">
                <a href="{{ route('register') }}" class="btn btn__primary--outlined">Daftar</a>
                <a href="{{ route('login') }}" class="btn btn__primary">Masuk</a>
            </div>
        </div>
    </div>
</nav>
